...
function Test() {
  ...
  useEffect(() => {
    if (positive.length > 0 && improvements.length > 0) {
      if (!isTaskComplete('/test')) markTaskComplete('/test');
    } else {
      if (isTaskComplete('/test')) markTaskUndone('/test');
    }
  }, [
    positive,
    improvements,
  ...
  return (
    <div>
      <Layout
        topContentTitle={'Task: Test Your Solution!'}
        topContentText={
          'Now it’s time to show your solution! Share it with your classmates, get their feedback, and see how they react.'
        }
        middleContent={
